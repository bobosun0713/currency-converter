...
const getMockSparkline = isUp => {
  return isUp ? "M0 30 Q 10 25, 20 28 T 40 20 T 60 25 T 80 15 L 100 10" : "M0 10 Q 30 15, 50 20 T 80 30 L 100 35";
};
  ...
  return currencyStore.favorites.map(fav => {
    ...
    return {
      ...
      fromName: fromCurrency
        ? `${fromCurrency.name}${fromCurrency.country ? ` (${fromCurrency.country})` : ""}`
        : fav.from,
  ...
  <div class="px-4 md:px-10 py-8 mx-auto w-full max-w-[1440px] relative">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 relative z-0">
      ...
      <div
        ...
        class="flex flex-col gap-4 rounded-xl p-6 border border-slate-200 dark:border-slate-700 bg-white dark:bg-surface-dark shadow-sm hover:shadow-md transition-shadow relative group"
      >
        ...
        <div class="flex justify-between items-start">
          <div class="flex items-center gap-3">
            <div>
              <p class="text-slate-900 dark:text-white text-base font-bold leading-tight pr-6">
                {{ card.from }} / {{ card.to }}
              </p>
        ...
        <div class="flex flex-wrap items-end gap-2 mt-2">
          ...
          <span
            :class="[
              'flex items-center text-sm font-medium mb-1 px-1.5 py-0.5 rounded shrink-0',
          ...
          >
            <span class="material-symbols-outlined text-[16px] mr-0.5">{{
              card.change.isUp ? "trending_up" : "trending_down"
            }}</span>
            {{ card.change.text }}
          </span>
        </div>
        ...
        <div class="h-16 w-full relative">
          <svg class="w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 100 40">
            <path
              :d="card.sparkline"
              fill="none"
              :class="card.change.isUp ? 'stroke-emerald-500' : 'stroke-rose-500'"
    ...
    >
      <div
        class="bg-white dark:bg-surface-dark w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200"
      >
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex flex-col gap-2 searchable-select">
              ...
              <div class="relative">
                ...
                <ul
                  ...
                  class="absolute z-50 w-full mt-1 max-h-48 overflow-y-auto bg-white dark:bg-neutral-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg"
                >
                  <li
          ...
          <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
            <p class="text-sm text-blue-700 dark:text-blue-300">
              您將追蹤 <strong>{{ newFrom }}</strong> 轉換為 <strong>{{ newTo }}</strong> 的即時匯率。
            </p>
          </div>
        </div>
