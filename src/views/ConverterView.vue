...
const filteredFromCurrencies = computed(() => {
  ...
  return allCurrencies.value.filter(
    c =>
...
<template>
  <div class="max-w-5xl mx-auto w-full flex flex-col gap-8 p-4 md:p-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      ...
      <div class="text-right hidden md:block">
        ...
        <div class="flex items-center gap-2 justify-end">
          ...
          <span v-if="currencyStore.lastUpdated" class="text-sm font-bold text-slate-900 dark:text-white">{{
    ...
    <!-- Converter Card -->
    <div
      class="bg-white dark:bg-surface-dark rounded-2xl shadow-sm border border-slate-200 dark:border-border-dark p-6 md:p-8"
    >
      <div class="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 items-end mb-8 relative z-10">
        ...
        <div class="flex flex-col gap-2">
          ...
          >
            ...
            <div
              class="w-1/2 md:w-[40%] text-slate-900 dark:text-white relative cursor-pointer group"
              @click="openDropdown('from')"
            >
              <div
                class="min-w-0 bg-transparent border-none outline-none font-bold p-4 flex items-center justify-between h-full"
              >
              ...
              <div
                v-show="isFromDropdownOpen"
                class="absolute top-full right-0 mt-2 w-72 bg-white dark:bg-neutral-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-50 overflow-hidden"
                @click.stop
              >
        ...
        <div class="flex flex-col gap-2">
          ...
          <div
            :class="[
              'flex rounded-xl border border-slate-200 dark:border-border-dark bg-slate-50 dark:bg-background-dark transition-all searchable-select relative',
              hasConverted
          ...
          >
            <!-- Output field -->
            <div
              class="w-1/2 md:w-[60%] min-w-0 bg-transparent border-none outline-none text-slate-900 dark:text-white text-xl font-bold p-4 flex items-center"
            >
              <span v-if="!hasConverted" class="text-slate-400 font-normal">0.00</span>
              <span v-else class="text-emerald-600 dark:text-emerald-400 font-bold transition-all">{{
                convertedAmount
              }}</span>
            </div>
      ...
      <!-- Result & Action -->
      <div
        class="flex flex-col md:flex-row items-center justify-between gap-6 pt-6 border-t border-slate-100 dark:border-border-dark relative z-0"
      >
