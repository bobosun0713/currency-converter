...
const exportToCSV = () => {
  ...
  const rows = filteredAndSortedData.value.map(item => [
    item.name,
    item.code,
    item.rate.toFixed(4),
    item.change.text
    ...
    <div class="flex flex-col max-w-[1200px] flex-1 w-full gap-6">
      ...
      <div class="flex flex-wrap justify-between items-end gap-4 pb-2">
        <div class="flex min-w-72 flex-col gap-2">
          <h1 class="text-slate-900 dark:text-white text-3xl md:text-4xl font-black leading-tight tracking-[-0.033em]">即時匯率</h1>
          <p class="text-slate-500 dark:text-slate-400 text-base font-normal leading-normal">主要世界貨幣對美金的即時匯率。</p>
        </div>
      ...
      <div class="flex flex-col sm:flex-row gap-4 items-center">
        <label class="flex flex-col h-12 w-full sm:flex-1 relative group">
          <div class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 group-focus-within:text-primary transition-colors">
        ...
        <div class="flex gap-2 w-full sm:w-auto">
          ...
          <div class="relative flex-1 sm:flex-none">
            <select v-model="sortOption" @change="currentPage = 1" class="appearance-none w-full h-12 pl-10 pr-10 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-slate-700 dark:text-slate-300 font-medium hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors focus:outline-none cursor-pointer">
          ...
          <!-- Export Button -->
          <button @click="exportToCSV" class="flex-1 sm:flex-none flex items-center justify-center gap-2 h-12 px-4 rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-700 text-slate-700 dark:text-slate-300 transition-colors shrink-0">
      ...
      <!-- Rates Table -->
      <div class="@container w-full overflow-x-auto rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-surface-dark shadow-sm">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50 dark:bg-background-dark border-b border-neutral-200 dark:border-neutral-700">
              <th class="px-4 py-4 w-12 text-center text-slate-400">
                <span class="material-symbols-outlined text-[18px]">push_pin</span>
              </th>
              <th class="px-4 py-4 text-slate-900 dark:text-white text-sm font-semibold leading-normal w-[30%]">貨幣名稱</th>
              <th class="px-4 py-4 text-slate-900 dark:text-white text-sm font-semibold leading-normal w-[20%]">代碼</th>
              <th class="px-4 py-4 text-slate-900 dark:text-white text-sm font-semibold leading-normal text-right w-[25%]">匯率 (1 USD =)</th>
              <th class="px-6 py-4 text-slate-900 dark:text-white text-sm font-semibold leading-normal text-right">24小時變動</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-neutral-200 dark:border-border-dark">
            <tr v-if="currencyStore.isFetching" class="group hover:bg-neutral-50 dark:hover:bg-background-dark transition-colors">
               <td colspan="5" class="px-6 py-8 text-center text-slate-500">正在載入即時資料...</td>
            </tr>
            <tr v-else-if="paginatedData.length === 0" class="group hover:bg-neutral-50 dark:hover:bg-background-dark transition-colors">
               <td colspan="5" class="px-6 py-8 text-center text-slate-500">沒有找到符合的貨幣</td>
            </tr>
            <tr v-else v-for="item in paginatedData" :key="item.code" class="group hover:bg-neutral-50 dark:hover:bg-background-dark transition-colors" :class="{'bg-primary/5 dark:bg-primary/10': item.isPinned}">
              <td class="px-4 py-4 text-center">
                <button @click="currencyStore.togglePin(item.code)" class="transition-colors hover:text-amber-500 focus:outline-none" :class="item.isPinned ? 'text-amber-500' : 'text-slate-300 dark:text-slate-600'">
                  <span class="material-symbols-outlined text-[18px]" :class="item.isPinned ? 'font-solid' : ''">push_pin</span>
                </button>
              </td>
              <td class="px-4 py-4">
              ...
              <td class="px-6 py-4 text-right">
                <div :class="[
                  ...
                  item.change.isUp ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 text-rose-600 dark:text-rose-400'
                ]">
                  <span class="material-symbols-outlined text-sm">{{ item.change.isUp ? 'trending_up' : 'trending_down' }}</span>
                  {{ item.change.text }}
                </div>
              </td>
            </tr>
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 pb-8">
        <div class="text-sm text-slate-500 dark:text-slate-400 order-2 sm:order-1 text-center sm:text-left">
          顯示第 <span class="font-bold text-slate-900 dark:text-white">{{ paginatedData.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0 }}</span> 到 <span class="font-bold text-slate-900 dark:text-white">{{ Math.min(currentPage * itemsPerPage, filteredAndSortedData.length) }}</span> 筆，共 <span class="font-bold text-slate-900 dark:text-white">{{ filteredAndSortedData.length }}</span> 筆資料
        </div>
